...
public class MergeSort implements MainClass{
    private int[] values;

    @Override
    public int chooseSort(int[] values, int searchVal) {
        int[] nueVals = margeSort(values);
        return binarySeacrh(values,0,values.length -1,searchVal);
    }

    private int[] margeSort(int arr[]){
        if (arr.length == 1)
            return arr;
        ...
        arrAux1 = module2.maintask.MergeSort.margeSort(arrAux1);
        arrAuc2 = module2.maintask.MergeSort.margeSort(arrAuc2);

        return merge(arrAux1,arrAuc2);
    }

    private int[] merge(int[] arrAux1, int[] arrAuc2) {
        int j = 0;
        int i = 0;
        int k =0;
